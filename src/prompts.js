...
Identify ALL significant grammar issues — up to 100 — but 100 is a CEILING, not a target: only REAL errors, never invent or pad to reach a number; if the writing is clean, return few or an empty array (the NO FABRICATION rule above still holds). Each grammar issue MUST include example_wrong and example_correct showing a DIFFERENT sentence than the student's to illustrate the rule. Also give up to 6 style observations and up to 8 vocabulary upgrades (same honesty — only real ones).
