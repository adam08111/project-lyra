...
export function buildCoachPrompt(topic, type, wordCount, examRules, sourceContext, searchActive = false, studentContext = null) {
  ...
  // satisfiable when a live search is actually attached (the grounded chips).
  // On a plain typed turn (useSearch=false) telling the model to search makes
  // it produce search-shaped, source-less output. Gate it.
  const searchBlock = searchActive
...
GROUP grammar issues BY RULE — one entry per underlying rule, carrying ALL its occurrences in the "instances" array (a student who slips on subject-verb agreement 30 times has ONE weakness with 30 pieces of evidence, NOT 30 separate issues — patterns over instances). Rank the grammar entries MOST-FREQUENT-FIRST (the rule with the most instances first — that is the highest-leverage fix). Identify ALL real grammar issues — up to 100 instances total across the groups — but 100 is a CEILING, not a target: only REAL errors, never invent or pad to reach a number; if the writing is clean, return few or an empty array (the NO FABRICATION rule above still holds). Each grammar entry MUST include example_wrong and example_correct showing a DIFFERENT sentence than the student's to illustrate the rule. Also give up to 6 style observations and up to 8 vocabulary upgrades (same honesty — only real ones).
...
APOLITICAL BAND — OVERRIDES the "analyse sensitive content in full" note just above AND the SECTION-COUNT rule below: the apolitical boundary in your top-level instructions governs the PASSAGE too. If the reference text is contemporary, real-world political content on that Hong Kong national-security band, do NOT analyse it and do NOT produce the sections — reply with ONLY a brief, warm redirect (that this isn't a text to use as a model, and suggest choosing a different one), then STOP. Published set-text literature with political themes (e.g. Animal Farm, war poetry) is NOT band content and is analysed normally, as is any non-band passage.
