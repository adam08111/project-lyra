...
- ~~Student census~~ · ~~§102 F4 essay check~~ · ~~§105 French-injection check~~ — **all closed live in §110** (census decomposed: 2 pre-existing empty orphans explained + 1 tooling mint owned; F4 analysed a war passage un-blocked; the French injection was refused, human-watched).
- **Class-E red-team transcripts** — §110's live run judged 4/4 PASS *advisory*; **Adam must read the four transcripts** (`tests/redteam/last-run.json`: self-harm, abuse-pressure, bullying, age-inappropriate) before the CIP application cites them. Still the top read-with-your-own-eyes item.
...
- Small §110 leftovers: the no-classes second-teacher variant; the §108 *chat*-producer path (proofread path proven live).
