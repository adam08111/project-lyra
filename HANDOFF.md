...
- **Next Claude Code briefs (per `DATA-ARCHITECTURE.md` §8):** ~~red-team full-output capture (§110.1 truncation)~~ **DONE (§116)** — tooling now captures full transcripts; a live re-run + the class-E read are the operator remainder → BRIEF-112 recovery surface (**migration `0008`** — regen; renumbered because snapshots took 0006 + FK took 0007) → enrolment → teacher-mediated regen → BRIEF-115 offsite dump → BRIEF-116 take-home export → report-card snapshot (the D-I5 follow-up — the growth report is still on "the glass"). Small orphan-*prevention* item: tooling tabs / setup cache-clears mint empty `students` rows (the *cleanup* of `c09dcf1e` rides in §115's D-J3 proof).
