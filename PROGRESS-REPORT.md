...
**Step 0 divergence (reported, benign):** the brief expected the §-log tip at §94.3; it was actually **§94.1** (two anticipated sub-entries were never created). The landing number **§95 is unaffected**. All *material* Step-0 checks matched exactly: `main.jsx` boot chain (`autoRestoreFromBackup → purgeInauthenticGrowthV1 → migrateTruncatedTitlesV1 → mount`), the 7 dedup-key shapes in `learning-sync.js`, and the three proof tests all as specified.
