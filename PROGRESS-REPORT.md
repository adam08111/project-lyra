...
## 131. UPDATE — 20 July 2026 — DATA-ARCHITECTURE §8 build-order refresh: un-freezing the §114 queue (CHECKPOINTS Lane-D doc debt)

`DATA-ARCHITECTURE.md §8` ("Build order — the canonical queue") was frozen at **§114-state** — "**Operator, parallel: apply migrations 0006 + 0007 + 0008**" plus a "Claude Code — still pending" list of briefs that have all since landed. §130 logged it as the one noticed-but-out-of-scope ghost (D-K4). This refresh un-freezes it. A **reorientation** session (not a resume of prior work): the maintainer re-synced to `origin/main` (tip `33b93db`/§130), reoriented off `CLAUDE.md` / `HANDOFF.md` / `CHECKPOINTS.md` / the § tail (§119–§130), and picked this item from the board — no brief invented. **Docs-only — zero `src/`/`api/`/`tests/`; no prompt, no migration, no product code.** §-tip was §130 → this is **§131**.

**Step 0 — the world confirmed against the tree, and the staleness is broader than the ghost.** Verified in the worktree (`claude/sync-reorient-main-d2977a` at `33b93db` = `origin/main`; the primary `main` checkout already level, nothing to FF): migrations `0006`–`0011` all exist on disk (`supabase/migrations/`), with `0006`–`0009` applied+verified live at §119 and `0010`/`0011` authored-not-applied (CHECKPOINTS A9); `DEPLOY.md:132` already carries the authoritative apply-order (`0006 → … → 0011`); Lane C is drained (only C5 flag-ON-via-staging remains, Vercel-A4-gated). **Scope call (Karpathy — smallest correct diff):** the SAME §119-freeze also lives in §0/§1/§3 as pre-§119 `BUILT-UNVERIFIED` labels on `0006`–`0008` (and §0's "last LIVE sitting §110" anchor), but flipping all of those + rewriting §0's dated ratification anchor is a **whole-document currency sweep** — only §8 was ghost-logged, so it stays the item and the §0/§1/§3 sweep is logged as a NEW Lane-D ghost. The one coherence fix §8 forced (§7 item 1 still called `0007` `BUILT-UNVERIFIED until applied`, which would flatly contradict a refreshed §8) rode along, per the §129 cross-ref-coherence precedent.

**The change (doc-only, redirect-not-duplicate).** *`DATA-ARCHITECTURE.md §8`* rewritten to mirror §129's HANDOFF-§5 treatment: the flat pending queue (which now duplicates `CHECKPOINTS.md`, created after §114) is **redirected to the board + `DEPLOY.md` + the § log** (single-source, CLAUDE.md #3), and §8 keeps only what a bare board doesn't carry — the **migration ledger** (`0006`–`0009` live §119; `0010`/`0011` authored-pending A9) and the **sequencing rule** (data-layer migrations → identity productization → flag-ON → pilot; the single-user crash-test done §119, the 40-concurrent pilot crash-test the biggest open risk; Identity v2 behind the PDPO consent framework). *§7 item 1* — the frozen `0007` status flipped to "applied + verified live §119 (the `23503` refusal witnessed by hand)" and the retention-posture check updated to §119 R6 (no anon auto-cleanup setting exists → the summer-purge threat is not active; RESTRICT is defense-in-depth; residual **F-R6** = raise the 30/hr/IP anon cap before a pilot). *§9 + header* — the successor-package transcript filename reconciled `identity-conversation-2026-07.txt` → `identity-conversation.md` to match the §130 drop-slot `docs/decisions/README.md` (single source). *`CHECKPOINTS.md`* — the §130 doc-debts ghost line ticked (§8 refreshed, §131) + the §0/§1/§3 whole-document sweep logged as the new Lane-D ghost.

**Verification — independent fact-check + suite + build.** Full suite **701 passed (76 files)** — matches the recorded count; docs-only, green by construction. `vite build` clean (both entry points). **Independent read-only fact-check** (a separate agent, adversarial — try to disprove each claim against `PROGRESS-REPORT §119–§130` + `CHECKPOINTS` + `DEPLOY` + the migration files + git): **all 11 load-bearing claims CONFIRMED** with `file:line` citations (0006–0009-live-§119, the `23503` witness, 0010/0011 authored-pending-A9, the DEPLOY apply-order, Lane-C-drained/C5, the D-Q5 class-P-FAIL fast-follow, the A4/A7/A9/A10 + Lane-B-3-Aug map, the §119-R6 retention posture, and the §0/§1/§3-labels-still-stale ghost). It caught **two findings, both fixed before landing:** (1) my §8→README pointer newly sharpened a filename mismatch (§9/header `.txt` vs the README's `.md`) — reconciled to `identity-conversation.md`; (2) the §8 header stamp "Refreshed §130" read as an off-by-one landing number — restyled to "Refreshed 20 Jul 2026, through §130" (matching §0's date convention; the landing is §131).

**Landing record.** Prior tip `33b93db` (§130). One logical commit — this refresh: `DATA-ARCHITECTURE.md` (§8 rewrite + the §7-item-1 coherence fix + the §9/header filename) + `CHECKPOINTS.md` (the doc-debts tick + the new §0/§1/§3 ghost) + this PROGRESS-REPORT §131. Per the **FINAL-TIP rule (CLAUDE.md #2)** this docs/log commit is the FINAL tip — no close-out follows. FF-land to `origin/main` **awaits the maintainer's same-session go-ahead** (CLAUDE.md #2 — never land without it); on approval `merge-base --is-ancestor origin/main HEAD` holds (this commit is a child of `33b93db`) and `git push origin HEAD:main` makes it the new `origin/main` (sha reported at hand-off; the FF condition re-verified against a fresh `origin/main` immediately before, never force), then the `Downloads\lyra-dev` main folder is fast-forwarded. No migration, no code, no shared-constant change. **Still open (Lane D):** the `DATA-ARCHITECTURE.md` §0/§1/§3 whole-document currency pass (the pre-§119 `BUILT-UNVERIFIED` labels + §0's "last LIVE sitting §110" anchor).
---

## 132. UPDATE — 21 July 2026 — A4 Vercel deploy landed (operator-reported): Lyra's first deployment exists (CHECKPOINTS reconciliation)

The operator (Adam) registered Vercel and stood up Lyra's **first live deployment** — **Hobby, flag-OFF** (`GEMINI_API_KEY` + `GATE_PASS` only; no `VITE_*`, so the Supabase mirror stays off and `/teacher.html` correctly shows not-configured). This records that milestone on the board. **No code, no migration, no `src/` — a CHECKPOINTS reconciliation** of an operator status update, not a Claude Code brief. §-tip was §131 → this is **§132**.

**What is and isn't done (no-fabrication — operator-reported, not loop-verified).** A4 is marked **partial (`[~]`)**, not closed: the deploy is live (Hobby, flag-OFF), but the **two §102 spot-checks** (security headers present; a fresh browser challenged by `GATE_PASS`) are **not yet run** (operator-confirmed), and the **phone test + MFA-at-signup** were not confirmed this exchange. A4 closes when the §102 spot-checks pass and MFA is confirmed on — the operator accounts are the crown jewels. **Downstream now un-gated:** C5 (flag-ON via staging) loses its "Vercel must exist first" gate, though it still needs Supabase **Pro** + the 30/hr/IP anon-cap raise before it is runnable; and the §130 `VERIFY at A4` doc tags (DEPLOY flag-state, the Hobby function-timeout cap, "production default", DATA-MAP's Vercel logging posture) are now **resolvable** — a follow-on doc pass once the deploy URL and the §102 spot-checks are in hand.

**Landing record.** Prior tip `867c2c2` (§131). One docs/log commit — `CHECKPOINTS.md` (the A4 partial tick) + this PROGRESS-REPORT §132. Per the **FINAL-TIP rule (CLAUDE.md #2)** this docs/log commit is the FINAL tip — no close-out follows. FF-land to `origin/main` **awaits the maintainer's same-session go-ahead**; on approval `git push origin HEAD:main` (`merge-base --is-ancestor` re-verified against a fresh `origin/main`, never force) makes it the new `origin/main` (sha reported at hand-off), then the `Downloads\lyra-dev` main folder is fast-forwarded. No migration, no code, no shared-constant change.
---

## 133. UPDATE — 22 July 2026 — ai-router.js cost-comment correction: the PRO/FLASH prices were stale (and the FLASH alias has inverted)

The `ai-router.js` header carried a stale cost block — "Pro ~$1.25/$10 · Flash $0.50/$3 · Flash-Lite $0.25/$1.50" — that no longer matches Google's pricing. Surfaced while answering a maintainer pricing question (gemini-3.6-flash vs Lyra's tiers). **Comment-only — one `src/` file; no behaviour, no routing, no model id changed.** §-tip was §132 → this is **§133**.

**Verified against Google's official pricing + models pages (July 2026):**
- **Pro** (`gemini-3-flash-preview`) is **$0.50 / $3.00** (cache $0.05), **not** $1.25/$10 — a Flash-*priced* model despite the "pro" label (the old figure predated `MODELS.pro` moving off a real Gemini-Pro-class model). Confirmed still listed/available.
- **Flash** (`gemini-flash-latest`) is a **rolling alias** to the latest stable Flash — currently **Gemini 3.5/3.6 Flash, ≈ $1.50 / $7.50–9.00** — so the FLASH tier now costs **more than PRO**, an inversion the old "$0.50/$3" hid. Exact figure depends on the alias's current resolution (Google gives 2-week email notice on alias rolls; the models page doesn't publish the mapping).
- **Lite** (`gemini-3.1-flash-lite`) is **$0.25 / $1.50** (cache $0.025) — the one line already correct; unchanged.

**Two follow-ups flagged (NOT done — these are behaviour/decision changes, the maintainer's call):** (1) the `welcome` route's "deliberately FLASH so the greeting is cheap — it fires on every session open" rationale is likely **inverted** now (flash ≥ pro), so the greeting may be the *priciest* small call — worth re-routing (lite, or pin pro). (2) `gemini-flash-latest` being a silent rolling alias is a cost + model-training-ToS liability for a children's app; pinning concrete model ids (as `pro`/`lite` already are) would make both cost and the training-data posture predictable.

**Verification.** 701 green (comment-only — no test surface); `git diff --stat` = `src/ai-router.js` only (+12/−4). Every figure cross-checked against Google's official pricing + models pages this session (WebFetch).

**Landing record.** Prior tip `92b8c52` (§132). One commit — `src/ai-router.js` (the cost-comment correction) + this PROGRESS-REPORT §133. Per the **FINAL-TIP rule (CLAUDE.md #2)** this is the FINAL tip — no close-out follows. FF-land to `origin/main` **awaits the maintainer's same-session go-ahead**; on approval `git push origin HEAD:main` (`merge-base --is-ancestor` re-verified against a fresh `origin/main`, never force) makes it the new `origin/main` (sha reported at hand-off), then the `Downloads\lyra-dev` main folder is fast-forwarded. No migration, no shared-constant change.
