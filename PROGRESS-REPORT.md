...
**Landing record.** Two logical commits — the RS feature (migration `0008` + `content-hash.js` + `report-snapshots.js` + `sync-outbox.js` + `growth-report.js` + `writing-snapshots.js` + tests) and the docs/log sync (this §117 + DEPLOY + DATA-ARCHITECTURE + SECURITY + HANDOFF). **Push/land OFFERED** (maintainer controls origin/main, CLAUDE.md #2 + the standing "user controls pushes" rule). When landed FF, the new `origin/main` sha is recorded in a §117 close-out. Lands at the main tip (§116 lineage); migration `0008` (BRIEF-ENROL, next, takes `0009`).
