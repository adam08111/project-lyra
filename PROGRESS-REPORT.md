...
**Landing record.** Commits (per unit — config seam+guard / teacher client / switch / initSync surface / tests / docs / log): to be listed at land. **Push/land is OFFERED, not executed** (maintainer controls origin/main; now also encoded in CLAUDE.md #2). When landed FF, record the new `origin/main` sha here.
