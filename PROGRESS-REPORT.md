...
**Landing record.** Two logical commits — the enrolment feature (migration `0009` + `src/enrol/*` + the `lyra.jsx` leaf mount + the D-L6 teacher line + the D-L7 seed + tests) and the docs/log sync (this §118 + DEPLOY + DATA-ARCHITECTURE §8 + SECURITY + HANDOFF). **Push/land OFFERED** (maintainer controls origin/main). When landed FF, the new `origin/main` sha is recorded in a §118 close-out. Lands at the main tip (§117 lineage); migration `0009` (BRIEF-112 recovery renumbers to `0010`).
